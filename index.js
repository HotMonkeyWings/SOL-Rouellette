...
const gameExecution = async () => {
    figlet('SOL Stake', (err, data) => {
        ...
        inquirer.prompt(questions).then((answers) => {
            ...
            transferSOL(toWallet, fromWallet, SOLamt).then((signature) => {
                console.log(
                    chalk.green(
                        'You will get',
                        returnAmount,
                ...
                inquirer
                    ...
                    .then((answers) => {
                        ...
                        if (randomGuess == answers['guess']) {
                            console.log(
                                chalk.green('Your guess is absolutely correct')
                            );
                            airDropSol(
                                fromWallet.publicKey,
                                returnAmount - SOLamt
                            ).then(
                                transferSOL(fromWallet, toWallet, returnAmount).then((winSign) => {
                                    console.log("Here is the price signature", chalk.green(winSign))
                                })
                            );
                        } else {
